...
function changeAccordionImage(input, divAccordion) {
     var accordion = document.getElementById(divAccordion);
    accordion.innerHTML = '';

    if (input instanceof FileList) {
        Array.from(input).forEach((file, index) => createAccordionElements(accordion, file, index, true));
    } else if (Array.isArray(input)) {
        Array.from(input).forEach((file, index) => createAccordionElements(accordion, file, index, false));
    } else {
